...
pub struct MapPutTransientCodec;

impl MapPutTransientCodec {

    // hex: 0x010D00
    const REQUEST_MESSAGE_TYPE: i32 = 68864;
    // hex: 0x010D01
    ...
    pub fn encode_request<'a>(name: &'a String, key: &'a HeapData, value: &'a HeapData, thread_id: &'a i64, ttl: &'a i64) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
