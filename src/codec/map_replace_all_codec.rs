...
pub struct MapReplaceAllCodec;

impl MapReplaceAllCodec {

    // hex: 0x014800
    const REQUEST_MESSAGE_TYPE: i32 = 83968;
    // hex: 0x014801
    // RESPONSE_MESSAGE_TYPE = 83969

    const REQUEST_INITIAL_FRAME_SIZE: usize = ClientMessage::PARTITION_ID_OFFSET as usize + BitsUtil::INT_SIZE_IN_BYTES as usize;

    pub fn encode_request<'a>(name: &'a String, function: &'a HeapData) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
            ...
            DataCodec::encode(&mut client_message, function).await;
