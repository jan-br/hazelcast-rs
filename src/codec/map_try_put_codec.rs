...
pub struct MapTryPutCodec;

impl MapTryPutCodec {

    // hex: 0x010C00
    const REQUEST_MESSAGE_TYPE: i32 = 68608;
    // hex: 0x010C01
    ...
    pub fn encode_request<'a>(name: &'a String, key: &'a HeapData, value: &'a HeapData, thread_id: &'a i64, timeout: &'a i64) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
