...
pub struct MapExecuteOnAllKeysCodec;

impl MapExecuteOnAllKeysCodec {

    // hex: 0x013000
    const REQUEST_MESSAGE_TYPE: i32 = 77824;
    // hex: 0x013001
    ...
    pub fn encode_request<'a>(name: &'a String, entry_processor: &'a HeapData) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
