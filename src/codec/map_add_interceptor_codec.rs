...
pub struct MapAddInterceptorCodec;

impl MapAddInterceptorCodec {

    // hex: 0x011400
    const REQUEST_MESSAGE_TYPE: i32 = 70656;
    // hex: 0x011401
    ...
    pub fn encode_request<'a>(name: &'a String, interceptor: &'a HeapData) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
            ...
            DataCodec::encode(&mut client_message, interceptor).await;
    ...
    pub fn decode_response<'a>(client_message: &'a mut ClientMessage) -> Pin<Box<dyn Future<Output=String> + Send + Sync + 'a>> {
        Box::pin(async move {
            // empty initial frame
            client_message.next_frame().await.unwrap();

            StringCodec::decode(client_message).await
        })
    }
