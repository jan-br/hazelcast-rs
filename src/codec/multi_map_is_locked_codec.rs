...
pub struct MultiMapIsLockedCodec;

impl MultiMapIsLockedCodec {

    // hex: 0x021200
    const REQUEST_MESSAGE_TYPE: i32 = 135680;
    // hex: 0x021201
    ...
    pub fn encode_request<'a>(name: &'a String, key: &'a HeapData) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
