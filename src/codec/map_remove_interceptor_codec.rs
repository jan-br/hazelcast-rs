...
pub struct MapRemoveInterceptorCodec;

impl MapRemoveInterceptorCodec {

    // hex: 0x011500
    const REQUEST_MESSAGE_TYPE: i32 = 70912;
    // hex: 0x011501
    ...
    pub fn encode_request<'a>(name: &'a String, id: &'a String) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
            ...
            StringCodec::encode(&mut client_message, id).await;
