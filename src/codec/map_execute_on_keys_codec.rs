...
pub struct MapExecuteOnKeysCodec;

impl MapExecuteOnKeysCodec {

    // hex: 0x013200
    const REQUEST_MESSAGE_TYPE: i32 = 78336;
    // hex: 0x013201
    ...
    pub fn encode_request<'a>(name: &'a String, entry_processor: &'a HeapData, keys: &'a Data[]) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
