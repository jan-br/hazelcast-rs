...
pub struct MapSizeCodec;

impl MapSizeCodec {

    // hex: 0x012A00
    const REQUEST_MESSAGE_TYPE: i32 = 76288;
    // hex: 0x012A01
    ...
    pub fn decode_response<'a>(client_message: &'a mut ClientMessage) -> Pin<Box<dyn Future<Output=i32> + Send + Sync + 'a>> {
        Box::pin(async move {
            ...
            FixSizedTypesCodec::decode_int(&*initial_frame.content.lock().await, Self::RESPONSE_RESPONSE_OFFSET).await
        })
    }
