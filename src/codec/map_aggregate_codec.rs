...
pub struct MapAggregateCodec;

impl MapAggregateCodec {

    // hex: 0x013900
    const REQUEST_MESSAGE_TYPE: i32 = 80128;
    // hex: 0x013901
    ...
    pub fn encode_request<'a>(name: &'a String, aggregator: &'a HeapData) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
