...
pub struct MultiMapUnlockCodec;

impl MultiMapUnlockCodec {

    // hex: 0x021300
    const REQUEST_MESSAGE_TYPE: i32 = 135936;
    // hex: 0x021301
    ...
    const REQUEST_REFERENCE_ID_OFFSET: usize = Self::REQUEST_THREAD_ID_OFFSET + BitsUtil::LONG_SIZE_IN_BYTES as usize;
    const REQUEST_INITIAL_FRAME_SIZE: usize = Self::REQUEST_REFERENCE_ID_OFFSET + BitsUtil::LONG_SIZE_IN_BYTES as usize;

    pub fn encode_request<'a>(name: &'a String, key: &'a HeapData, thread_id: &'a i64, reference_id: &'a i64) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
