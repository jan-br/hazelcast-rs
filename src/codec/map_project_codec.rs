...
pub struct MapProjectCodec;

impl MapProjectCodec {

    // hex: 0x013B00
    const REQUEST_MESSAGE_TYPE: i32 = 80640;
    // hex: 0x013B01
    ...
    pub fn encode_request<'a>(name: &'a String, projection: &'a HeapData) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
