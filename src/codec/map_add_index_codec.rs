...
use crate::config::InternalIndexConfig;
use crate::codec::custom::custom::IndexConfigCodec;
...
pub struct MapAddIndexCodec;

impl MapAddIndexCodec {

    // hex: 0x012900
    const REQUEST_MESSAGE_TYPE: i32 = 76032;
    // hex: 0x012901
    ...
    pub fn encode_request<'a>(name: &'a String, index_config: &'a InternalIndexConfig) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
            ...
            IndexConfigCodec::encode(&mut client_message, index_config).await;
