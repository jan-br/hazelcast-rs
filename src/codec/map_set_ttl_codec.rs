...
pub struct MapSetTtlCodec;

impl MapSetTtlCodec {

    // hex: 0x014300
    const REQUEST_MESSAGE_TYPE: i32 = 82688;
    // hex: 0x014301
    // RESPONSE_MESSAGE_TYPE = 82689

    const REQUEST_TTL_OFFSET: usize = ClientMessage::PARTITION_ID_OFFSET as usize + BitsUtil::INT_SIZE_IN_BYTES as usize;
    const REQUEST_INITIAL_FRAME_SIZE: usize = Self::REQUEST_TTL_OFFSET + BitsUtil::LONG_SIZE_IN_BYTES as usize;
    ...
    pub fn encode_request<'a>(name: &'a String, key: &'a HeapData, ttl: &'a i64) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
