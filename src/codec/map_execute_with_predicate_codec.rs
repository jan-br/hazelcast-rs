...
pub struct MapExecuteWithPredicateCodec;

impl MapExecuteWithPredicateCodec {

    // hex: 0x013100
    const REQUEST_MESSAGE_TYPE: i32 = 78080;
    // hex: 0x013101
    ...
    pub fn encode_request<'a>(name: &'a String, entry_processor: &'a HeapData, predicate: &'a HeapData) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
