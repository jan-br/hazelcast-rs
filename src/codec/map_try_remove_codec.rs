...
pub struct MapTryRemoveCodec;

impl MapTryRemoveCodec {

    // hex: 0x010B00
    const REQUEST_MESSAGE_TYPE: i32 = 68352;
    // hex: 0x010B01
    ...
    const REQUEST_TIMEOUT_OFFSET: usize = Self::REQUEST_THREAD_ID_OFFSET + BitsUtil::LONG_SIZE_IN_BYTES as usize;
    const REQUEST_INITIAL_FRAME_SIZE: usize = Self::REQUEST_TIMEOUT_OFFSET + BitsUtil::LONG_SIZE_IN_BYTES as usize;
    ...
    pub fn encode_request<'a>(name: &'a String, key: &'a HeapData, thread_id: &'a i64, timeout: &'a i64) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
