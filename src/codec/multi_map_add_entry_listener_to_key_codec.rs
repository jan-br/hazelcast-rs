...
pub struct MultiMapAddEntryListenerToKeyCodec;

impl MultiMapAddEntryListenerToKeyCodec {

    // hex: 0x020D00
    const REQUEST_MESSAGE_TYPE: i32 = 134400;
    // hex: 0x020D01
    // RESPONSE_MESSAGE_TYPE = 134401
    // hex: 0x020D02
    const EVENT_ENTRY_MESSAGE_TYPE: i32 = 134402;

    const REQUEST_INCLUDE_VALUE_OFFSET: usize = ClientMessage::PARTITION_ID_OFFSET as usize + BitsUtil::INT_SIZE_IN_BYTES as usize;
    const REQUEST_LOCAL_ONLY_OFFSET: usize = Self::REQUEST_INCLUDE_VALUE_OFFSET + BitsUtil::BOOLEAN_SIZE_IN_BYTES as usize;
    ...
    pub fn encode_request<'a>(name: &'a String, key: &'a HeapData, include_value: &'a bool, local_only: &'a bool) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
