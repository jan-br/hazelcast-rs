...
pub struct MapValuesWithPagingPredicateCodec;

impl MapValuesWithPagingPredicateCodec {

    // hex: 0x013500
    const REQUEST_MESSAGE_TYPE: i32 = 79104;
    // hex: 0x013501
    ...
    pub fn decode_response<'a>(client_message: &'a mut ClientMessage) -> Pin<Box<dyn Future<Output=MapValuesWithPagingPredicateResponseParams> + Send + Sync + 'a>> {
        Box::pin(async move {
            // empty initial frame
