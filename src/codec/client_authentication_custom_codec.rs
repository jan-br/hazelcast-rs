...
pub struct ClientAuthenticationCustomResponseParams {
    pub status: u8,
...
pub struct ClientAuthenticationCustomCodec;

impl ClientAuthenticationCustomCodec {

    // hex: 0x000200
    const REQUEST_MESSAGE_TYPE: i32 = 512;
    // hex: 0x000201
    ...
    pub fn encode_request<'a>(cluster_name: &'a String, credentials: &'a Buffer, uuid: &'a Option<&'a Uuid>, client_type: &'a String, serialization_version: &'a u8, client_hazelcast_version: &'a String, client_name: &'a String, labels: &'a Vec<String>) -> Pin<Box<dyn Future<Output=ClientMessage> + Send + Sync + 'a>> {
        Box::pin(async move {
            ...
            ByteArrayCodec::encode(&mut client_message, credentials).await;
    ...
    pub fn decode_response<'a>(client_message: &'a mut ClientMessage) -> Pin<Box<dyn Future<Output=ClientAuthenticationCustomResponseParams> + Send + Sync + 'a>> {
        Box::pin(async move {
